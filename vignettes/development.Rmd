...
| {murmur} | Due to the complexity of how the project was coming together, I decided to split the major functions into separate packages. Path/relationships were a different primary concept than modeling, and the API didn't feel smooth. Thus, "Many Unexplained Relationships Modeled Using R" was created. |
| {armour} | The key feature or class was the concept of the `model list`. This was the most recurrent challenge in that groups of models contain shared information. Visualizing how these relationships evolved in different modeling approaches should be informative in learning about underlying mechanisms. |
...
A purpose of having parallel and sequential model sets, built into a hypothesis, was to identify confounding variables and help remove them.

## Fifth Draft

Hopefully the final draft. I simplified the API to exclude the complexity of path analysis and focused just on the hypotheses and fitting models. I also removed the complexity of formula management, and placed that into a separate package. 
