test_that("a formula list can be fit internally to create a model construct", {
	...
	# Create model suit
	out <- model_construct(x, fitting_function = lm, data = mtcars)
	expect_length(out, 3)
	expect_output(print(out), "model_construct")

})


test_that("several independent models can be added together in a list/construct", {
